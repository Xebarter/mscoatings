...
import { getProductByBarcode, getProducts, type Product } from '@/lib/firestore';
import type { FieldAgent } from '@/lib/erp-types';
...
export default function NewPickModal({
  ...
  const handleSubmitPick = async () => {
    ...
    try {
      ...
      toast.success(`Pick created for ${selectedAgent.name}`);
      onSuccess(pickId);
      onClose();
    } catch (error) {
                    ...
                    agents.map((agent) => (
                      ...
                      >
                        ...
                        <p className="mt-2 text-xs text-slate-400">
                          {agent.totalPicks} picks · {formatUgx(agent.totalRevenue)} revenue
                        </p>
                      </button>
                    ))
                  )}
